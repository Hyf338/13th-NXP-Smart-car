...
uint8 circle_right=0;//左环右环
uint8 circle_left=0 ;
uint8 speed_count=0;
uint8 dirction_count=0;
uint8 close_round=0;//圆环总开关
uint8 round_status[4]={1,3,1,4};
...
void PIT_CH0_IRQHandler(void)
{
  ...
  if(direction_stop_flag==0)  //触发钢黄管停车
  {
    ...
    if(speed_count==10) //20ms采一次1ms的中断
    {
      speed_count=0;
  ...
  if(dirction_count==2)
  {
    ...
    if(close_round==0)
    {
      ...
      case 1:
        {
          if(ser_middle<95&&round_flag==1&&round_turn==1)
          {
            round_flag=0;
            round_turn=0;
            out_flag=0;
      ...
      if(round_flag==1&&circle_right==0&&circle_left==0)//等待拐点
      {
      ...
      {
        if(straight_right<straight_left)  round_turn=1;//拐点位置
      }
      if(circle_left==1&&round_flag==1)//&&ser_middle>180
      {
        if(straight_left<straight_right)   round_turn=1 ;//&&(ser_left>ser_right)拐点位置
      }
      
      if(circle_right==1&&out_flag==0&&ser_right>120&&ser_middle>110)  out_round=1;//出环再遇峰值
      if(out_round==1&&ser_right<85&&circle_right==1) //环内状态
      {
        out_round=0;
        out_flag=1;
        circle_right=0;
      ...
      if(circle_left==1&&out_flag==0&&ser_left>120&&ser_middle>110)  out_round=1;//出环再遇峰值
      ...
      if(round_turn==0&&count_flag==1)//环计数
      {
        count_flag=0;
        round_count+=1;
      }
      if(round_turn==1)//入圆环算法
      {
        count_flag=1 ;
        switch(speed_switch)
        {
        case 1:
          {
            switch(round_status[round_count-1])//设置圆环参数
            {
        ...
        case 2:
          {
            ...
            case 4: round_hight=235,round_low=210;break;
            case 5: round_hight=230,round_low=205;break;
        ...
        case 3:
          {
            ...
            case 2: round_hight=230,round_low=ser_middle+15;break;
            case 3: round_hight=220,round_low=ser_middle+10;break;
            case 4: round_hight=240,round_low=215;break;
            case 5: round_hight=230,round_low=210;break;
            default: round_hight=235,round_low=215;break;
            }
          }
        default:break;
    ...
    if(actual_speed<250&&close_flag==1)
      direction_fix=1.09;
    else
      direction_fix=1;
    ...
    case 3:
      {
        Motor_left=(int16)(motor_pulse_speed-direction_fix*ser_duty);
        Motor_right=(int16)(motor_pulse_speed+direction_fix*ser_duty);
        break ;
      }
    default:break;
    }
    ...
    if(start_flag==1)//  &&close_flag==1
    {
      ...
      if( Motor_left<0)
      {
        // Motor_left=(int16)(direction_fix*Motor_left);
        ftm_pwm_duty(ftm2,ftm_ch2,0);
        ftm_pwm_duty(ftm2,ftm_ch3,-Motor_left);
      }
      if( Motor_right<0)
      {
        // Motor_right=(int16)(direction_fix*Motor_right);
